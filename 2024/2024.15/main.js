...
const gridstr = inp[0].split("\n");
const gridOne = gridstr.map(x => x.split(""));
const gridTwo = (inp[0].replaceAll("#", "##").replaceAll(".", "..").replaceAll("O", "[]").replaceAll("@", "@.")).split("\n").map(x => x.split(""))
...
function move(grid, fx, fy, dx, dy, apply = true){
	...
	if (tx == 0 || ty == 0)
		return false;
	if (tx == grid[0].length || ty == grid.length)
		return false;
...
console.log("part one :", simulate(gridOne));
console.log("part two :", simulate(gridTwo));
